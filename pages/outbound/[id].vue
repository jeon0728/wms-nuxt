...
onMounted(async () => {
  loading.value = true
  item.value = await store.get(Number(route.params.id))
  loading.value = false
})

const markReceived = async () => {
  await store.update(Number(route.params.id), { status: 'pending' })
...
<template>
  <div class="p-6">
    <h1 class="text-xl font-semibold mb-4">Outbound Detail</h1>
    ...
    <div v-else-if="item">
      ...
      <div class="mt-4">
        ...
        <NuxtLink to="/outbound" class="ml-2 px-3 py-2 border rounded">목록</NuxtLink>
      </div>
    </div>
